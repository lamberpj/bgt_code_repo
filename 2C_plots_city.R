...
# df$year_month <- as.yearmon(df$job_date)
# df$year <- year(df$job_date)
# colnames(df)
# 
# df <- df %>%
#   setDT(.)
...
df_cit_large
p = ggplot(df_cit_large, aes(x = city_country_fac, y = wfh_share, fill = as.character(period))) +
...
set.seed(999)
df_cit
uniqueN(paste0(df_cit$city, df_cit$state)) # 36,597
df_cit_scat <- df_cit %>%
  select(-prop_growth) %>%
  .[, size_keep := ifelse(any(N < 10000) | any(wfh_share == 0), 0, 1), by = .(city, state, country)] %>%
  .[size_keep == 1] %>%
  select(-size_keep)
uniqueN(paste0(df_cit_scat$city, df_cit_scat$state)) # 554
df_cit_scat <- df_cit_scat %>%
...
ts_for_plot_cit <- ts_for_plot_cit %>%
  mutate(city_state = case_when(
    city_state == "San Francisco, California" ~ "01 San Francisco, California",
    city_state == "Boston, Massachusetts" ~ "02 Boston, Massachusetts",
    city_state == "Phoenix, Arizona" ~ "03 Phoenix, Arizona",
    city_state == "Denver, Colorado" ~ "04 Denver, Colorado",
    city_state == "New York, New York" ~ "05 New York, New York",
    city_state == "Jacksonville, Florida" ~ "06 Jacksonville, Florida",
    city_state == "Houston, Texas" ~ "07 Houston, Texas",
    city_state == " US National" ~ "08 US National",
    city_state == "Louisville, Kentucky" ~ "09 Louisville, Kentucky",
    city_state == "Memphis, Tennessee" ~ "10 Memphis, Tennessee",
    city_state == "Savannah, Georgia" ~ "11 Savannah, Georgia",
    city_state == "Miami Beach, Florida" ~ "12 Miami Beach, Florida",
    TRUE ~ city_state,
  ))
...
p
save(p, file = "./ppt/ggplots/ts_cities_ne_w_s.RData")
