...
df_us <- df_us %>% select(wfh_wham, country, state, year_month, job_date)

df_all_list <- list(df_us)
...
fwrite(daily_data, file = "./aux_data/all_states_daily.csv")
...
# Load all data
remove(list = ls())
daily_data <- fread(file = "./aux_data/all_states_daily.csv")
head(daily_data)
check <- daily_data %>%
  .[, .(N = sum(N)), by = .(country, state)]
...
ts_for_plot <- ts_for_plot %>%
  .[order(state, job_date)] %>%
  .[, monthly_mean_l1o := sum(l1o_daily_with_nas[!is.na(l1o_daily_with_nas)]*N[!is.na(l1o_daily_with_nas)], na.rm = T)/(sum(N[!is.na(l1o_daily_with_nas)], na.rm = T)), by = .(state, year_month)] %>%
  .[, .(N = sum(N),
        job_date = min(job_date),
        drop_days_share = sum(1-l1o_keep)/.N,
        monthly_mean = monthly_mean[1],
        monthly_mean_l1o = monthly_mean_l1o[1]),
    by = .(country, state, year_month)] %>%
  .[, monthly_mean_3ma := rollmean(monthly_mean, k = 3, align = "right", fill = NA), by = .(country, state)] %>%
    .[, monthly_mean_3ma_l1o := rollmean(monthly_mean_l1o, k = 3, align = "right", fill = NA), by = .(country, state)] %>%
...
fwrite(ts_for_plot, file = "./aux_data/state_level_ts.csv")
