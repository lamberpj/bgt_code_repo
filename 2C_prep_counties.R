...
max(df_uk_2023$job_date)
max(df_us_2023$job_date)
...
df_uk <- df_uk %>% select(wfh_wham, country, state, county, fips, year_month, job_date)
df_us <- df_us %>% select(wfh_wham, country, state, county, fips, year_month, job_date)
...
daily_data <- lapply(1:length(df_all_list), function(i) {
  df_all_list[[i]] %>%
    .[county != "" & !is.na(county)] %>%
    .[, .(daily_share = mean(wfh_wham, na.rm = T), N = .N), by = .(country, state, county, fips, year_month, job_date)] %>%
...
ts_for_plot <- daily_data %>%
  ...
  .[, l1o_monthly_mean := (sum(daily_share*N)-daily_share*N)/(sum(N) - N), by = .(country, county, fips, state, county_state, year_month)] %>%
  .[, monthly_mean := sum(daily_share*N)/(sum(N)), by = .(country, county, fips, state, county_state, year_month)] %>%
...
ts_for_plot <- ts_for_plot %>%
  .[order(county, fips, state, county_state, job_date)] %>%
  .[, monthly_mean_l1o := sum(l1o_daily_with_nas[!is.na(l1o_daily_with_nas)]*N[!is.na(l1o_daily_with_nas)], na.rm = T)/(sum(N[!is.na(l1o_daily_with_nas)], na.rm = T)), by = .(county, fips, state, county_state, year_month)] %>%
  .[, .(N = sum(N),
        job_date = min(job_date),
        drop_days_share = sum(1-l1o_keep)/.N,
        monthly_mean = monthly_mean[1],
        monthly_mean_l1o = monthly_mean_l1o[1]),
    by = .(country, county, fips, state, county_state, year_month)] %>%
  .[, monthly_mean_3ma := rollmean(monthly_mean, k = 3, align = "right", fill = NA), by = .(country, county, fips, state, county_state)] %>%
    .[, monthly_mean_3ma_l1o := rollmean(monthly_mean_l1o, k = 3, align = "right", fill = NA), by = .(country, county, fips, state, county_state)] %>%
...
ts_for_plot <- ts_for_plot %>%
  group_by(county, fips, state, county_state, year_month, job_date, N) %>%
