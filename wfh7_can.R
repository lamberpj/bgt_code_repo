...
#### EXTRACT URL AND SOURCE FOR CAN ####
# remove(list = ls())
paths <- list.files("./raw_data/text/", pattern = "*.zip", full.names = T)
paths
source("/mnt/disks/pdisk/bgt_code_repo/safe_mclapply.R")
df_src <- safe_mclapply(1:length(paths), function(i) {
  ...
  unlink(xml_path)
  warning(paste0("FAIL: ",i))
  return("")
}, mc.cores = 4)
# df_src <- df_src[-c(33,48)]
# lapply(df_src, is_empty)
...
#### DICT ####
df_dict <- readRDS("/mnt/disks/pdisk/bg-can/int_data/CAN_full_results.rds")
df_dict <- df_dict %>% select(-sequence)
head(df_dict)
df_dict <- df_dict %>%
...
remove(list = ls())
df_can_stru_2014 <- fread("../bg-can/int_data/can_stru_2014_wfh.csv", nThread = 8) %>% .[!is.na(wfh_wham)]
df_can_stru_2015 <- fread("../bg-can/int_data/can_stru_2015_wfh.csv", nThread = 8) %>% .[!is.na(wfh_wham)]
df_can_stru_2016 <- fread("../bg-can/int_data/can_stru_2016_wfh.csv", nThread = 8) %>% .[!is.na(wfh_wham)]
df_can_stru_2017 <- fread("../bg-can/int_data/can_stru_2017_wfh.csv", nThread = 8) %>% .[!is.na(wfh_wham)]
df_can_stru_2018 <- fread("../bg-can/int_data/can_stru_2018_wfh.csv", nThread = 8) %>% .[!is.na(wfh_wham)]
df_can_stru_2019 <- fread("../bg-can/int_data/can_stru_2019_wfh.csv", nThread = 8) %>% .[!is.na(wfh_wham)]
df_can_stru_2020 <- fread("../bg-can/int_data/can_stru_2020_wfh.csv", nThread = 8) %>% .[!is.na(wfh_wham)]
...
#df_all_can <- split(df_all_can, by="year_month")

# lapply(1:length(df_all_can), function(i) {
#   fwrite(df_all_can[[i]], file = paste0("../bg_combined/int_data/standardised_quarterly/bg_uk_",gsub(" ", "_",tolower(names(df_all_can)[[i]])),".csv"))
# })
