...
system("gsutil -m cp -r gs://for_transfer/bgt-can/Main /mnt/disks/pdisk/bg-can/raw_data/main/")
system("gsutil -m cp -r gs://for_transfer/bgt-can/XML /mnt/disks/pdisk/bg-can/raw_data/text/")
#
#library(filesstrings)
paths <- list.files("/mnt/disks/pdisk/bg-can/raw_data/main/Main", full.names = T, pattern = ".zip", recursive = T)
paths
...
colnames(fread(paths[97], nrows = 100))
fread(paths[98], nrows = 100)
...
df_stru <- safe_mclapply(1:length(paths), function(i) {
  ...
  df <- fread(paths[i], nThread = 2, colClasses = "character", stringsAsFactors = FALSE,
              select = c("JobID",  "JobDate", "BGTOcc", "CanonCity")) %>%
    clean_names %>%
    setDT(.)
  ...
  df <- df %>%
    .[!is.na(bgt_occ) & !is.na(canon_city)] %>%
...
nrow(df_stru_ss)/nrow(df_stru) # 0.362154
...
safe_mclapply(1:length(paths), function(i) {
  ...
  df_ss_sequence <- df_ss_sequence[!duplicated(seq_id)]
...
saveRDS(df, file = "./int_data/training_v6/can_training_v6_unclustered.rds")
