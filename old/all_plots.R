...
usa_weights <- df_all %>%
  ...
  .[, .(us_tot_emp = sum(tot_emp_ad),
        us_tot_vacs = sum(job_id_weight)),
    by = .(bgt_occ)]

within_country_weights <- df_all %>%
  setDT(.) %>%
  .[year == 2019] %>%
  .[, .(wc_tot_emp = sum(tot_emp_ad),
        wc_tot_vacs = sum(job_id_weight)),
    by = .(country, bgt_occ)]

# Merge in weights
nrow(df_all)
df_all <- df_all %>%
  left_join(usa_weights)
nrow(df_all)

nrow(df_all)
df_all <- df_all %>%
  left_join(within_country_weights)
nrow(df_all)
...
#  group_by(country) %>%
#  select(country, year_month, share_bert_emp) %>%
#  summarise(mean_share = 100*mean(share_bert_emp, na.rm = T))
#
#wfh_ag_month_spike <- wfh_ag %>%
#  filter(year_month %in% as.yearmon(c("Feb 2020", "Apr 2020", "Apr 2021"))) %>%
#  group_by(country) %>%
#  select(country, year_month, share_bert_emp) %>%
#  mutate(share_bert_emp = share_bert_emp*100) %>%
#  pivot_wider(., names_from = year_month, values_from = share_bert_emp) %>%
...
p = wfh_ag %>%
  ggplot(., aes(x = as.Date(year_month), y = 100*share_bert_wc_emp, col = country)) +
...
p_egg <- set_panel_size(p = p,
                        width = unit(5, "in"),
                        height = unit(3, "in"))
ggsave(p_egg, filename = "./plots/bert_wfh_lr_ts_emp2019_a.pdf", width = 8, height = 6)
...
p_egg <- set_panel_size(p = p,
                        width = unit(5, "in"),
                        height = unit(3, "in"))
ggsave(p_egg, filename = "./plots/bert_wfh_lr_index_emp2019_a.pdf", width = 8, height = 6)
...
# Unweighted
p = wfh_ag %>%
  ggplot(., aes(x = as.Date(year_month), y = 100*share_bert_unweighted, col = country)) +
  #stat_smooth (geom="line", alpha=0.8, size=2, span=0.2) +
  #stat_smooth(span = 0.1, alpha=0.5, se=FALSE, size = 2) +
  geom_point(size = 2.25) +
  geom_line(size = 1.25) +
...
p_egg <- set_panel_size(p = p,
                        width = unit(5, "in"),
                        height = unit(3, "in"))
ggsave(p_egg, filename = "./plots/bert_wfh_lr_ts_unweighted.pdf", width = 8, height = 6)
...
p = wfh_ag %>%
  ...
  ylab("Share (%)") +
  xlab("Date") +
  labs(title = "Share of Remote Work Vacancy Postings (%)", subtitle = "Vacancy Weighted") +
  #scale_x_yearmon(format = '%Y-%-m', breaks = seq(from = as.yearmon("2014-1"), to = as.yearmon("2021-9"), by = 1),  limits = c(as.yearmon("2014-1"), as.yearmon("2021-9"))) +
  scale_x_date(breaks = as.Date(c("2014-01-01", "2015-01-01", "2016-01-01", "2017-01-01", "2018-01-01", "2019-01-01", "2020-01-01", "2021-01-01", "2022-01-01", "2022-01-01")),
               minor_breaks = as.Date(c("2014-01-01","2014-07-01","2015-01-01","2015-07-01","2016-01-01","2016-07-01","2017-01-01","2017-07-01",
                                        "2018-01-01","2018-07-01","2019-01-01","2019-07-01","2020-01-01","2020-07-01","2021-01-01","2021-07-01", "2022-01-01")),
...
p_egg <- set_panel_size(p = p,
                        width = unit(5, "in"),
                        height = unit(3, "in"))
ggsave(p_egg, filename = "./plots/bert_wfh_lr_ts_vac_weighted.pdf", width = 8, height = 6)
...
p = wfh_ag %>%
  ...
  ylab("Share (%)") +
  xlab("Date") +
  labs(title = "Share of Remote Work Vacancy Postings (%)", subtitle = "USA Employment Weighted") +
  #scale_x_yearmon(format = '%Y-%-m', breaks = seq(from = as.yearmon("2014-1"), to = as.yearmon("2021-9"), by = 1),  limits = c(as.yearmon("2014-1"), as.yearmon("2021-9"))) +
  scale_x_date(breaks = as.Date(c("2014-01-01", "2015-01-01", "2016-01-01", "2017-01-01", "2018-01-01", "2019-01-01", "2020-01-01", "2021-01-01", "2022-01-01", "2022-01-01")),
               minor_breaks = as.Date(c("2014-01-01","2014-07-01","2015-01-01","2015-07-01","2016-01-01","2016-07-01","2017-01-01","2017-07-01",
                                        "2018-01-01","2018-07-01","2019-01-01","2019-07-01","2020-01-01","2020-07-01","2021-01-01","2021-07-01", "2022-01-01")),
...
p_egg <- set_panel_size(p = p,
                        width = unit(5, "in"),
                        height = unit(3, "in"))
ggsave(p_egg, filename = "./plots/bert_wfh_lr_ts_us_emp2019_a.pdf", width = 8, height = 6)
...
p = wfh_ag %>%
  ...
  ylab("Share (%)") +
  xlab("Date") +
  labs(title = "Share of Remote Work Vacancy Postings (%)", subtitle = "USA Vacancy Weighted") +
  #scale_x_yearmon(format = '%Y-%-m', breaks = seq(from = as.yearmon("2014-1"), to = as.yearmon("2021-9"), by = 1),  limits = c(as.yearmon("2014-1"), as.yearmon("2021-9"))) +
  scale_x_date(breaks = as.Date(c("2014-01-01", "2015-01-01", "2016-01-01", "2017-01-01", "2018-01-01", "2019-01-01", "2020-01-01", "2021-01-01", "2022-01-01", "2022-01-01")),
               minor_breaks = as.Date(c("2014-01-01","2014-07-01","2015-01-01","2015-07-01","2016-01-01","2016-07-01","2017-01-01","2017-07-01",
                                        "2018-01-01","2018-07-01","2019-01-01","2019-07-01","2020-01-01","2020-07-01","2021-01-01","2021-07-01", "2022-01-01")),
...
p_egg <- set_panel_size(p = p,
                        width = unit(5, "in"),
                        height = unit(3, "in"))
ggsave(p_egg, filename = "./plots/bert_wfh_lr_ts_us_vac2019_a.pdf", width = 8, height = 6)
