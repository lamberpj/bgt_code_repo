...
df_stru <- safe_mclapply(1:length(paths), function(i) {
  ...
  df <- fread(paths[i], data.table = F, nThread = 1, colClasses = "character", stringsAsFactors = FALSE,
              select = c("JobID", "JobDate", "BGTCareerAreaName", "UKSOCCode", "BGTOcc", "TTWA", "CanonCounty", "SICSection", "CanonEmployer")) %>%
    clean_names
  df[df == "na"] <- NA
  df[df == "-999"] <- NA
  df[df == ""] <- NA
  
  df <- df %>% mutate(keep1 = ifelse(!is.na(uksoc_code) & !is.na(canon_county), T, F))
  df <- df %>%
    mutate(soc2 = str_sub(bgt_occ, 1, 2)) %>%
    mutate(keep2 = ifelse(!(soc2 %in% drop_occ$soc2), T, F))
  warning(paste0("\nDONE: ",i))
  return(df)
}, mc.cores = 16)
...
nrow(df_stru[df_stru$keep1 == T & df_stru$keep2 == T,]) # 39,998,519
nrow(df_stru) # 61,026,425
n_distinct(df_stru$bgt_occ[df_stru$keep1 == T & df_stru$keep2 == T]) # 467
n_distinct(df_stru$bgt_occ) # 730
n_distinct(df_stru$canon_county[df_stru$keep1 == T & df_stru$keep2 == T]) # 241
n_distinct(df_stru$canon_county) # 241
n_distinct(df_stru$ttwa[df_stru$keep1 == T & df_stru$keep2 == T]) # 241
n_distinct(df_stru$ttwa) # 241

# UK: 40.0M (61.0M) job ads, 467 (730) BGT Occupations, 241 (242) Counties, 225 (225) TTWAs

stargazer(top_level, summary =  F, title = "New Data Selection (UK)", rownames = F)
...
stargazer(ss_occ_stats %>% select(soc2, label, n_share), summary =  F, title = "Dropped Occupations and their Shares (UK)", rownames = F)

colnames(df_stru)

ss_md_stats <- df_stru %>%
  mutate(n = 1) %>%
  rename(soc = uksoc_code,
         sector_name = sic_section,
         employer = canon_employer,
         county = canon_county,
         onet = bgt_occ) %>%
  select(n, soc, onet, county, sector_name, employer) %>%
  summarise(across(everything(), ~ sum(!is.na(.x), na.rm = TRUE))) %>%
  mutate(soc = round(1-(soc/n), 3),
         onet = round(1-(onet/n), 3),
         county = round(1-(county/n), 3),
         sector_name = round(1-(sector_name/n), 3),
         employer = round(1-(employer/n), 3)) %>%
  select(-n)

stargazer(ss_md_stats, summary =  F, title = "Missing Covariates and their Shares (UK)", rownames = F)
