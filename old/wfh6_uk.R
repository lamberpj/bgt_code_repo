...
system("gsutil -m cp -r gs://for_transfer/bgt-uk/Main /mnt/disks/pdisk/bg-uk/raw_data/main/")
system("gsutil -m cp -r gs://for_transfer/bgt-uk/XML /mnt/disks/pdisk/bg-uk/raw_data/text/")
#
#library(filesstrings)
paths <- list.files("/mnt/disks/pdisk/bg-uk/raw_data/main/Main", full.names = T, pattern = ".zip", recursive = T)
paths
...
nrow(df_stru_ss)/nrow(df_stru) # 0.256
...
wfh_intensity_v6_long <- wfh_intensity_v6 %>%
  ...
  .[, N := .N, by = key]
rm(wfh_intensity_v6)
nrow(wfh_intensity_v6_long) # 540
...
nrow(random_non_tagged) # 1,230
ls()

nrow(wfh_v6_long) # 1129
nrow(wfh_neg_v6_long) # 695
nrow(wfh_intensity_v6_long) # 540
nrow(excemptions_v6_long) # 835
nrow(generic_v6_long) # 888
nrow(random_non_tagged) # 1230
...
saveRDS(df, file = "./int_data/training_v6/uk_training_v6_unclustered.rds")
