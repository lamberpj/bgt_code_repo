...
# Get US 2021-07 and US 2022-01
paths <- paths[grepl("202102", paths)]
paths
...
all_raw <- safe_mclapply(1:length(paths), function(i) {
...
all_raw <- rbindlist(all_raw)

all_raw_new <- read_xlsx("./aux_data/rawjobinfo_text_sample.xlsx") %>% setDT(.)

remove(list = setdiff(ls(), c("all_raw_new", "all_raw")))

all_raw$job_id <- as.numeric(all_raw$job_id)
all_raw_new$bgtjobid <- as.numeric(all_raw_new$bgtjobid)

colnames(all_raw) <- paste0(colnames(all_raw),"_old")

all_raw_new <- all_raw_new %>%
  .[, nchar := nchar(job_posting_text)] %>%
  .[, nfeat := str_count(job_posting_text, '\\w+')]

all_raw_new <- all_raw_new[bgtjobid %in% all_raw$job_id_old]
all_raw <- all_raw[job_id_old %in% all_raw_new$bgtjobid]

all_raw_both <- all_raw_new %>%
  left_join(., all_raw, by = c("bgtjobid" = "job_id_old"))

colnames(all_raw)

sum(all_raw_both$nchar != all_raw_both$nchar_old, na.rm = T) / nrow(all_raw_both)
sum(all_raw_both$nfeat != all_raw_both$nfeat_old, na.rm = T) / nrow(all_raw_both)

all_raw_both$nchar_diff <- all_raw_both$nchar - all_raw_both$nchar_old
all_raw_both$nfeat_diff <- all_raw_both$nfeat - all_raw_both$nfeat_old

quantile(all_raw_both$nchar - all_raw_both$nchar_old, probs = seq(0,1,0.05))
quantile(all_raw_both$nfeat - all_raw_both$nfeat_old, probs = seq(0,1,0.05))

View(all_raw_both[nchar != nchar_old, c("job_posting_text", "job_text_old")])

all_raw_both$job_posting_text2 <-  gsub("[\r\n]", " ", all_raw_both$job_posting_text)
all_raw_both$job_posting_text2 <-  gsub("\\s+", " ", all_raw_both$job_posting_text2)

all_raw_both$job_text_old2 <-  gsub("[\r\n]", " ", all_raw_both$job_text_old)
all_raw_both$job_text_old2 <-  gsub("\\s+", " ", all_raw_both$job_text_old2)

all_raw_both$nchar2 <- nchar(all_raw_both$job_posting_text2)
all_raw_both$nchar_old2 <- nchar(all_raw_both$job_text_old2)

fwrite(all_raw_both, "./raw_data/new_text/compare_old_new_bledi_sample.csv")
