...
df_stru <- safe_mclapply(1:length(paths), function(i) {
  ...
  df <- fread(paths[i], nThread = 2, colClasses = "character", stringsAsFactors = FALSE,
              select = c("JobID", "JobDate", "BGTOcc", "TTWA")) %>%
    clean_names %>%
    setDT(.)
...
nrow(df_stru_ss)/nrow(df_stru) # 0.01000005
df_stru_ss <- df_stru_ss %>% rename(bgt_job_id = job_id)
...
save(df_stru_ss_plus, file = "./aux_data/df_stru_ss_plus_v5_uk.RData")
...
paths_check <- list.files("./raw_data/text/", pattern = "*.zip", full.names = T) %>%
  str_sub(., -21, -5)
paths[!(paths_check %in% paths_done)]
...
safe_mclapply(1:length(paths), function(i) {
  ...
  try( {
    ...
    df_chunked <- df_ads %>%
      ...
      #.[, job_text_clean := replace_ordinal(job_text_clean)] %>%
      #.[, job_text_clean := replace_number(job_text_clean)] %>%
      #.[, job_text_clean := replace_symbol(job_text_clean)] %>%
      #.[, job_text_clean := replace_word_elongation(job_text_clean)] %>%
      #.[, job_text_clean := replace_contraction(job_text_clean)] %>%
      ...
      .[, id := ifelse(id_real == 1, 1, id), by = job_id] %>%
      .[, id := nafill(id, type = "locf"), by = job_id] %>%
      .[, id := nafill(id, type = "nocb"), by = job_id] %>%
      .[, cs := cumsum(nfeat) %/% 50, by = .(job_id, id)] %>%
      .[, id := id+(cs/100)] %>%
      .[, .(para = paste0(para, collapse = "\n")), by = .(job_id, id)] %>%
      .[, nchar := nchar(para)] %>%
      .[, nfeat := str_count(para, '\\w+')] %>%
      .[, large := ifelse(nfeat > 50, TRUE, FALSE)] %>%
      .[, sentence := ifelse(large == TRUE, tokenize_sentences(para), para)] %>%
      ...
      .[, id_real := 1:.N, by = job_id] %>%
      .[, id := ifelse(nfeat < 30, NA, id_real)] %>%
      .[, id := ifelse(is.na(id), shift(id)+1, id), by = job_id] %>%
      .[, id := ifelse(id_real == 1, 1, id), by = job_id] %>%
      .[, id := nafill(id, type = "locf"), by = job_id] %>%
      .[, id := nafill(id, type = "nocb"), by = job_id] %>%
      .[, cs := cumsum(nfeat) %/% 60, by = .(job_id, id)] %>%
      .[, id := id+(cs/100)] %>%
      .[, .(sentence = paste0(sentence, collapse = " "), n = .N), by = .(job_id, id, large)] %>%
      .[, nchar := nchar(sentence)] %>%
      .[, nfeat := str_count(sentence, '\\w+')] %>%
      .[, large := ifelse(nfeat > 100, TRUE, FALSE)] %>%
      .[, frag := ifelse(large == TRUE, strsplit(as.character(sentence), "(?<=[[:punct:]])", perl = T), sentence)] %>%
      unnest(cols = frag) %>%
      setDT(.) %>%
      select(job_id, large, frag) %>%
      .[, nchar := nchar(frag)] %>%
      .[, nfeat := str_count(frag, '\\w+')] %>%
      .[, id_real := 1:.N, by = job_id] %>%
      .[, id := ifelse(nfeat < 30, NA, id_real)] %>%
      ...
      .[, cs := cumsum(nfeat) %/% 60, by = .(job_id, id)] %>%
      .[, id := id+(cs/100)] %>%
      .[, .(frag = paste0(frag, collapse = " "), n = .N), by = .(job_id, id, large)] %>%
      .[, nchar := nchar(frag)] %>%
      .[, nfeat := str_count(frag, '\\w+')] %>%
      .[, id_real := 1:.N, by = job_id] %>%
      .[, id := ifelse(nfeat < 30, NA, id_real)] %>%
      .[, id := nafill(id, type = "locf"), by = job_id] %>%
      .[, id := nafill(id, type = "nocb"), by = job_id] %>%
      .[, .(frag = paste0(frag, collapse = " "), n = .N), by = .(job_id, id)] %>%
      .[, nchar := nchar(frag)] %>%
      .[, nfeat := str_count(frag, '\\w+')] %>%
      .[, id_real := 1:.N, by = job_id] %>%
      .[, frag_id := paste0(job_id,"_",sprintf("%04d", as.numeric(id_real)))] %>%
      select(job_id, frag_id, frag, n, nfeat, nchar)
...
system("zip ./int_data/wfh_v4/wfh_v4_uk.zip -r ./int_data/wfh_v4")
