...
paths <- paths[grepl("202201", paths)]
paths
...
all_raw_new <- fread("./raw_data/new_text/US_2022-01.csv", integer64 = "numeric")

all_raw_new <- all_raw_new %>%
  rename(job_id = bgtjobid, job_text = job_posting_text) %>%
  .[, nchar := nchar(job_text)] %>%
  .[, nfeat := str_count(job_text, '\\w+')]



sum(all_raw_new$job_id %in% all_raw$job_id)
sum(all_raw$job_id %in% all_raw_new$job_id)

all_raw_new <- all_raw_new[job_id %in% all_raw$job_id]
all_raw <- all_raw[job_id %in% all_raw_new$job_id]
colnames(all_raw_new)[2:4] <- paste0(colnames(all_raw_new)[2:4], "_new")

all_raw_both <- all_raw %>%
  left_join(all_raw_new)

sum(all_raw_both$nchar != all_raw_both$nchar_new) / nrow(all_raw_both)
sum(all_raw_both$nfeat != all_raw_both$nfeat_new) / nrow(all_raw_both)

all_raw_both$nchar_diff <- all_raw_both$nchar - all_raw_both$nchar_new
all_raw_both$nfeat_diff <- all_raw_both$nfeat - all_raw_both$nfeat_new

quantile(all_raw_both$nchar - all_raw_both$nchar_new, probs = seq(0,1,0.001))
quantile(all_raw_both$nfeat - all_raw_both$nfeat_new, probs = seq(0,1,0.05))

View(all_raw_both[nfeat != nfeat_new])

all_raw_both_sam <- all_raw_both %>% sample_frac(size = 0.05)

sum(nchar(all_raw_both_sam$job_text) != nchar(all_raw_both_sam$job_text_new))

sum(all_raw_both_sam$nchar != all_raw_both_sam$nchar_new)

fwrite(all_raw_both_sam, "./raw_data/new_text/compare_old_new_sample_jan2022.csv")
