...
df_list <- safe_mclapply(1:m, function(i) {
  ...
  y1 <- fread(file_names$file_names[file_names$type == "postings"][i],
              data.table = F,
              nThread = 1,
              select = c("general_id","year_grab_date","month_grab_date","companyname","idcountry")) %>%
    clean_names %>%
  ...
  y2 <- y2 %>%
    group_by(year_grab_date,month_grab_date,companyname_clean,idcountry) %>%
...
df <- df %>%
  group_by(year_grab_date,month_grab_date,companyname_clean) %>%
  summarise(escoskill_level_3 = paste(escoskill_level_3, collapse = " "),
            n = sum(n)) %>%
  mutate(doc_id = paste0(year_grab_date,"_",month_grab_date,"_",companyname_clean))

sum(duplicated(df$doc_id)) # 2
df <- df %>% filter(!is.na(companyname_clean))
sum(duplicated(df$doc_id)) # 0



remove(list = setdiff(ls(),"df"))

save(df, file = "./eu_alt_firm_skill_text.RData")
remove(list = ls())
load(file = "./eu_alt_firm_skill_text.RData")
...
df <- df %>%
  ...
  mutate(n_posts_firm = sum(n)) %>%
  ungroup()

df <- df_filt
remove("df_filt")

summary(df$n_posts_firm)

### Keep firms with > 100 postings across all time
df <- df %>%
  filter(n_posts_firm > 100)
...
sd_df <- mclapply(1:ncol(df_dfm_prop_f), function(i) {
  sd(df_dfm_prop_f[,i])
}, mc.cores = 32) %>%
  unlist(.) %>%
  as.matrix()
...
is.positive.definite(cov_aj_mat, tol=1e-20)
...
docvars(df_dfm_f)$fyear_firm <- paste0(docvars(df_dfm_f)$fyear,"_",docvars(df_dfm_f)$companyname_clean)

df_dfm_f <- df_dfm_f %>% dfm_subset(., rowSums(df_dfm_f)>0)
df_dfm_f_fyear_firm <- df_dfm_f %>% dfm_group(groups = docvars(df_dfm_f)$fyear_firm, fill = FALSE, force = FALSE)
df_dfm_f_fyear_firm_prop <- df_dfm_f_fyear_firm %>% dfm_weight(., scheme = "prop")
df_dfm_f_fyear_firm_prop <- df_dfm_f_fyear_firm_prop * 100

#### MAHALANOBIS and ABSOLUTE DISTANCES ####
firm_list <- unique(docvars(df_dfm_f_fyear_firm_prop)$companyname_clean)

length(firm_list) # 100,851

df_dfm_f_fyear_firm_prop_listsplit <- safe_mclapply(firm_list, function(x) {
  dfm_subset(df_dfm_f_fyear_firm_prop, docvars(df_dfm_f_fyear_firm_prop)$companyname_clean == x)
}, mc.cores = 16)

length(df_dfm_f_fyear_firm_prop_listsplit) # 100,851

firm_list <- firm_list[lapply(df_dfm_f_fyear_firm_prop_listsplit, ndoc) > 5]
df_dfm_f_fyear_firm_prop_listsplit <- df_dfm_f_fyear_firm_prop_listsplit[lapply(df_dfm_f_fyear_firm_prop_listsplit, ndoc) > 5]
length(firm_list) # 75,785
length(df_dfm_f_fyear_firm_prop_listsplit) # 75,785
...
df_dist <- safe_mclapply(1:length(df_dfm_f_fyear_firm_prop_listsplit), function(i) {
  md_function(df_dfm_f_fyear_firm_prop_listsplit[[i]], inv_vcov = inv_vcov) %>%
    mutate(companyname_clean = firm_list[[i]])
...
save(df, file = "./us_firm_skill_text.RData")
remove(list = ls())
load(file = "./us_firm_skill_text.RData")
...
# Make fyear data
docvars(df_dfm_f)$fyear <- ifelse(docvars(df_dfm_f)$month_grab_date > 3, docvars(df_dfm_f)$year_grab_date, docvars(df_dfm_f)$year_grab_date-1)
docvars(df_dfm_f)$fyear_firm <- paste0(docvars(df_dfm_f)$fyear,"_",docvars(df_dfm_f)$soc)

tail(docvars(df_dfm_f))

df_dfm_f <- df_dfm_f %>% dfm_subset(., rowSums(df_dfm_f)>0)
df_dfm_f_fyear_firm <- df_dfm_f %>% dfm_group(groups = docvars(df_dfm_f)$fyear_firm, fill = FALSE, force = FALSE)
nfeat(df_dfm_f_fyear_firm)
df_dfm_f_fyear_firm_prop <- df_dfm_f_fyear_firm %>% dfm_weight(., scheme = "prop")
df_dfm_f_fyear_firm_prop <- df_dfm_f_fyear_firm_prop * 100

tail(docvars(df_dfm_f_fyear_firm_prop))

#### MAHALANOBIS and ABSOLUTE DISTANCES ####
occ_list <- unique(docvars(df_dfm_f_fyear_firm_prop)$soc)

df_dfm_f_fyear_firm_prop_listsplit <- lapply(occ_list, function(x) {
  df_dfm_f_fyear_firm_prop_example <- dfm_subset(df_dfm_f_fyear_firm_prop, docvars(df_dfm_f_fyear_firm_prop)$soc == x)
})
length(df_dfm_f_fyear_firm_prop_listsplit) # 837
occ_list <- occ_list[lapply(df_dfm_f_fyear_firm_prop_listsplit, ndoc) > 1]
df_dfm_f_fyear_firm_prop_listsplit <- df_dfm_f_fyear_firm_prop_listsplit[lapply(df_dfm_f_fyear_firm_prop_listsplit, ndoc) > 1]
length(occ_list) # 836
length(df_dfm_f_fyear_firm_prop_listsplit) # 836
...
df_dist <- lapply(1:length(df_dfm_f_fyear_firm_prop_listsplit), function(i) {
  print(i)
  md_function(df_dfm_f_fyear_firm_prop_listsplit[[i]], inv_vcov = inv_vcov) %>%
