...
df <- mclapply(1:m, function(i) {
  y1 <- lapply(file_names$file_names[file_names$type == "postings" & file_names$country == countries[[i]]], function(x) {
    fread(paste0(x), data.table = F, nThread = 1, select = c("general_id","year_grab_date","month_grab_date","day_grab_date",
                                                             "salary", "idregion", "region", "idesco_level_4", "esco_level_4",
                                                             "idmacro_sector", "macro_sector", "category_sector", "idcountry"))}) %>%
    bind_rows %>%
    clean_names
  
  y1[y1 == ""] <- NA
  y1 <- y1 %>% filter(!is.na(salary))
...
df <- df %>%
  mutate(salary_num = case_when(
    salary == "0 - 6.000 EUR" ~ 3,
    salary == "6.001 - 12.000 EUR" ~ 9,
    salary == "12.001 - 18.000 EUR" ~ 15,
    salary == "18.001 - 24.000 EUR" ~ 21,
    salary == "24.001 - 30.000 EUR" ~ 27,
    salary == "30.001 - 36.000 EUR" ~ 33,
    salary == "36.001 - 42.000 EUR" ~ 39,
    salary == "42.001 - 48.000 EUR" ~ 45,
...
df <- df %>% filter(as.numeric(str_sub(year_month, 1, 4)) > 2018)
...
#### DENSITIES ####
df_density <- df %>% select(salary_num, covid) %>% mutate(covid = ifelse(covid=="1pre","pre","covid"))

p <- ggplot(df_density, aes(x = salary_num, colour = covid)) + geom_density()
p

#### DEEMEAN ####
df1 <- df %>% mutate(date = as.Date(parse_date_time(year_month, "ym")))
df2 <- df %>% filter(!is.na(idesco_level_4)) %>% mutate(date = as.Date(parse_date_time(year_month, "ym")))
df3 <- df %>% filter(!is.na(idesco_level_4) & !is.na(idmacro_sector)) %>% mutate(date = as.Date(parse_date_time(year_month, "ym")))
df4 <- df %>% filter(!is.na(idesco_level_4) & !is.na(idmacro_sector) & !is.na(idregion)) %>% mutate(date = as.Date(parse_date_time(year_month, "ym")))

lm1 <- feols(salary_num ~ covid | idcountry, df1)
lm2 <- feols(salary_num ~ covid | idcountry + idesco_level_4, df2)
lm3 <- feols(salary_num ~ covid | idcountry + idesco_level_4 + idmacro_sector, df3)
lm4 <- feols(salary_num ~ covid | idcountry + idesco_level_4 + idmacro_sector + idregion, df4)
