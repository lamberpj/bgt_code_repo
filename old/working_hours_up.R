...
df <- mclapply(1:m, function(i) {
  y1 <- lapply(file_names$file_names[file_names$type == "postings" & file_names$country == countries[[i]]], function(x) {
    fread(paste0(x), data.table = F, nThread = 1, select = c("general_id","year_grab_date","month_grab_date","day_grab_date",
                                                             "idworking_hours", "working_hours", "idregion", "region", "idesco_level_4", "esco_level_4",
                                                             "idmacro_sector", "macro_sector", "category_sector", "idcountry"))}) %>%
    bind_rows %>%
    clean_names
  
  y1[y1 == ""] <- NA
  y1 <- y1 %>% filter(!is.na(idworking_hours))
...
df <- df %>% mutate(ft = ifelse(idworking_hours == 2,1,0))
...
lm1 <- feols(ft ~ covid + as.factor(month) | idcountry, df1)
lm2 <- feols(ft ~ covid + as.factor(month) | idcountry + idesco_level_4, df2)
lm3 <- feols(ft ~ covid + as.factor(month) | idcountry + idesco_level_4 + idmacro_sector, df3)
lm4 <- feols(ft ~ covid + as.factor(month) | idcountry + idesco_level_4 + idmacro_sector + idregion, df4)
